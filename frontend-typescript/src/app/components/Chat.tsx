...
const ChatInConversation: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Array<{ textData: string; sender: string }>>([]);
  ...
  const fetchComunications = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${apiURL}/conversation/get-comunication`, conversationData);
  ...
  const handleSendMessage = async () => {
    if (newMessage.trim() !== "") {
      setMessages([...messages, { textData: newMessage, sender: "user" }]);
      setNewMessage("");
      ...
      const response = await axios.post(`${apiURL}/sms/send-sms`, conversationData);
